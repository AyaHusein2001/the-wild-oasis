...
const SalesChart = ({ bookings, numDays }) => {
  ...
  // we need allDates even those that don't have any bookings, so we start from today(end), back 7 days or 30 or 90 ...
  const allDates = eachDayOfInterval({
    start: subDays(new Date(), numDays - 1),
    end: new Date(),
  });
  // for each day , get its bookings( bookings having same day as current day) , sum all those bookings total sales up to get the total sales for that day
...
const hasBookings = data.some(day => day.totalSales > 0);
